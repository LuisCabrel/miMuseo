...
/**
 * Created by nakis on 17/01/2018.
 */

public class PizarraKalmanEst extends View {

    private Bitmap miBitmap=null;
    public static Canvas miCanvasKalmanEst=null;
    ...
    public static Paint miPaintKalmanEstBalizas=null;
    public static Paint miPaintKalmanEstPersona=null;
    public static Paint miPaintKalmanEstIncertidumbre=null;
    public static Paint avisoPaint=null;
    public static ShapeDrawable sdCuadro=null;


    public PizarraKalmanEst(Context context) {
        super(context);
        iniciarPizarraYPincel(context);
    }

    public PizarraKalmanEst(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        iniciarPizarraYPincel(context);
    }
    ...
    private void iniciarPizarraYPincel(Context contexto){
        ...
        miCanvasKalmanEst=new Canvas(miBitmap);
        //miPath=new Path();
        miPoint=new Point();

        //parte del pincel que dibuja balizas
        miPaintKalmanEstBalizas=new Paint();
        miPaintKalmanEstBalizas.setStyle(Paint.Style.STROKE);
        miPaintKalmanEstBalizas.setStrokeJoin(Paint.Join.ROUND);
        miPaintKalmanEstBalizas.setStrokeCap(Paint.Cap.ROUND);
        miPaintKalmanEstBalizas.setAntiAlias(true);
        miPaintKalmanEstBalizas.setDither(true);
        miPaintKalmanEstBalizas.setColor(0xFFffcd00);
        miPaintKalmanEstBalizas.setStrokeWidth(20);

        //parte del pincel que dibuja a la posición de la persona
        miPaintKalmanEstPersona=new Paint();
        miPaintKalmanEstPersona.setStyle(Paint.Style.STROKE);
        miPaintKalmanEstPersona.setStrokeJoin(Paint.Join.ROUND);
        miPaintKalmanEstPersona.setStrokeCap(Paint.Cap.ROUND);
        miPaintKalmanEstPersona.setAntiAlias(true);
        miPaintKalmanEstPersona.setDither(true);
        miPaintKalmanEstPersona.setColor(0xFF0081cb);
        miPaintKalmanEstPersona.setStrokeWidth(20);
        ...
        miPaintKalmanEstIncertidumbre=new Paint();
        miPaintKalmanEstIncertidumbre.setStyle(Paint.Style.FILL_AND_STROKE);
        miPaintKalmanEstIncertidumbre.setStrokeJoin(Paint.Join.ROUND);
        miPaintKalmanEstIncertidumbre.setStrokeCap(Paint.Cap.ROUND);
        miPaintKalmanEstIncertidumbre.setAntiAlias(true);
        miPaintKalmanEstIncertidumbre.setDither(true);
        miPaintKalmanEstIncertidumbre.setColor(0x7769e2ff);
    ...
    protected void onDraw(Canvas canvas){
        ...
        recojo_mispx[0]=(int)(2* mis_pxMaek[0]/(float)3);//altura cuando está de pie
        if(depie()){
            recojo_mispx[1]=(int) mis_pxMaek[1];//ancho
        }else{
            recojo_mispx[1]=(int)(15* mis_pxMaek[1]/16);//ancho
        }
        ...
        float casoResolucion=((mis_pxMaek[1]-2*margen)/((2* mis_pxMaek[0]/3)-2*margen));
        ...
        if(depie()) {
            if (casoEscenario > casoResolucion) {
                //limita el ancho
                mirec = new RectF(margen, margen, (int)((mis_pxMaek[1] - 2 * (float)margen) + (float)margen), ((int)((recojo_mispx[1] - 2 * margen) * largo / (float)ancho) + margen));//destiny
                otrorec = new Rect(margen, margen, (int)((mis_pxMaek[1] - 2 * (float)margen) + (float)margen), ((int)((recojo_mispx[1] - 2 * margen) * largo / (float)ancho) + margen));//source

            } else {
                //limita el largo
                mirec = new RectF(margen, margen, ((int)(((2/(float)3)*mis_pxMaek[0] - 2 * margen) * ancho / (float)largo) + margen), ((recojo_mispx[0] - 2 * margen) + margen));//destiny
                otrorec = new Rect(margen, margen, ((int)(((2/(float)3)*mis_pxMaek[0] - 2 * margen) * ancho / (float)largo) + margen), ((recojo_mispx[0] - 2 * margen) + margen));//source
            }
        }else{
            mirec=new RectF(margen,margen,recojo_mispx[1]-margen,recojo_mispx[0]-margen);//destiny
            otrorec=new Rect(margen,margen,recojo_mispx[1]-margen,recojo_mispx[0]-margen);//source
        ...
        if(!depie()){
            canvas.drawText("Regrese a la posición vertical",56f,mis_pxMaek[0]/3,avisoPaint);
        }
